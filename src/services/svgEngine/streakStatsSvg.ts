...
export const renderStreakStatsSvg = (config: StreakStatsConfig & { userData?: GitHubUserData }): string => {
  ...
  // Real or fallback streak values
  const totalContributions = config.userData?.streak?.totalContributions ?? 1842;
  const currentStreak = config.userData?.streak?.currentStreak ?? 42;
  const longestStreak = config.userData?.streak?.longestStreak ?? 89;
  ...
  <g transform="translate(20, 20)">
    ...
    <text x="70" y="42" font-family="'Segoe UI', Ubuntu, sans-serif" font-size="24" font-weight="800" fill="${numberColor}" text-anchor="middle">${totalContributions.toLocaleString()}</text>
  ...
  <g transform="translate(177, 20)">
    ...
    <text x="70" y="42" font-family="'Segoe UI', Ubuntu, sans-serif" font-size="24" font-weight="800" fill="${numberColor}" text-anchor="middle">${currentStreak} <tspan font-size="13" font-weight="600">${labels.daysUnit}</tspan></text>
    ...
    <text x="70" y="85" font-family="'Segoe UI', Ubuntu, sans-serif" font-size="10" fill="${textColor}" opacity="0.6" text-anchor="middle">Active</text>
  </g>

  <!-- Stat Box 3: Longest Streak -->
  <g transform="translate(335, 20)">
    <rect x="0" y="0" width="140" height="110" rx="8" fill="${borderColor}" fill-opacity="0.15" stroke="${borderColor}" stroke-opacity="0.4"/>
    <text x="70" y="42" font-family="'Segoe UI', Ubuntu, sans-serif" font-size="24" font-weight="800" fill="${numberColor}" text-anchor="middle">${longestStreak} <tspan font-size="13" font-weight="600">${labels.daysUnit}</tspan></text>
    ...
    <text x="70" y="85" font-family="'Segoe UI', Ubuntu, sans-serif" font-size="10" fill="${textColor}" opacity="0.6" text-anchor="middle">Personal Best</text>
  </g>
</svg>`;
};
